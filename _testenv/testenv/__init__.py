...
top_dir = os.path.realpath(f"{__file__}/../../../..")
data_dir = os.path.join(os.path.realpath(f"{__file__}/../.."), "data")
distro_default = "debian:12"
...
def parse_args():
    global args
    parser = argparse.ArgumentParser(description="Build everything needed for"
                                     " running Osmocom TTCN-3 testsuite and"
                                     " execute them")
    ...
    group.add_argument("testsuite",
                          help="a directory in osmo-ttcn3-hacks.git (msc, bsc,"
                               " mgw, ...)")
    ...
    group.add_argument("-c", "--config",
                       help="which testenv.cfg to use, in case the testsuite"
                            " has multiple (e.g. generic|oml|hopping for bts)")

    group = sub_run.add_argument_group("source/binary options",
                                       "All components are built from source"
                                       " by default.")
    group = group.add_mutually_exclusive_group()
    group.add_argument("-b", "--binary-repo", metavar="OBS_PROJECT",
                       help="use binary packages from this Osmocom OBS project"
                            " instead (e.g. osmocom:nightly)")
    group.add_argument("-C", "--clone", action="store_true",
                       help="clone missing git repositories automatically")
    ...
    group = sub_run.add_argument_group("podman options",
                                      ...
                                      " host by default.")
    group.add_argument("-p", "--podman", action="store_true",
    ...
    group.add_argument("-s", "--shell", action="store_true",
                       help="run an interactive shell before stopping daemons/"
                            "container")
    ...
    sub_image = sub.add_parser("image", help="build the podman image (for run"
                                             " --podman)")
    sub_image.add_argument("-f", "--force", action="store_true",
                           help="build image even if it is up-to-date")

    sub_clean = sub.add_parser("clean", help="clean previous build artifacts")
...
class ColorFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if record.levelno == logging.DEBUG:
            self._style._fmt = f"{gray}[testenv] %(msg)s{end}"
        elif record.levelno == logging.INFO:
            self._style._fmt = f"{blue}[testenv] %(msg)s{end}"
        elif record.levelno == logging.WARNING:
            self._style._fmt = f"{yellow}[testenv] %(msg)s{end}"
        elif record.levelno == logging.ERROR:
            self._style._fmt = f"{red}[testenv] %(msg)s{end}"
