...
def apt_install(pkgs):
    ...
    testenv.cmd.run(["apt-get", "install", "-y", "--no-install-recommends"] + pkgs)
...
def from_source_sccp_demo_user():
    ...
    if not os.path.exists(sccp_demo_user_path):
        ...
        testenv.cmd.run(["make", "-j", f"{jobs}", "libosmo-sigtran.la"],
                        cwd=os.path.join(sccp_dir, "src"))
        testenv.cmd.run(["make", "-j", f"{jobs}", "sccp_demo_user"],
