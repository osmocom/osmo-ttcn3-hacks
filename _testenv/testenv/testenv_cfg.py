...
def find_configs():
    ...
    if len(ret) > 1 and not testenv.args.config:
        ...
        logging.error(f"Select a specific config (e.g. '-c {example}')"
                      " or all ('-c all')")
        sys.exit(1)
...
def check_if_clone_is_needed():
    for cfg_name, cfg in cfgs.items():
        for section in cfg:
            if "repo" not in cfg[section]:
                continue
            repo = cfg[section]["repo"]
            repo_path = os.path.join(testenv.top_dir, repo)
            if os.path.exists(repo_path):
                continue

            logging.error(f"Repository dir for [{section}] not found!")
            logging.error(f"* Use '-C' to clone {repo}.git and its"
                          f" dependencies to: {testenv.top_dir}")
            logging.error("* Use '-pb osmocom:nightly' to"
                          " use binary packages instead of building from"
                          " source")
            sys.exit(1)
    ...
    for path in config_paths:
        ...
        if basename != "testenv.cfg" and not basename.startswith("testenv_"):
            raise testenv.NoTraceException("Invalid filename, expected either"
                                         f" testenv.cfg or testenv_*.cfg: {cfg}")
        ...
        config_arg = testenv.args.config
        if not config_arg or config_arg == "all" or f"testenv_{config_arg}.cfg" == basename:
            cfgs[basename] = cfg

    if not cfgs:
        raise_error_config_arg(config_paths)

    if not testenv.args.binary_repo and not testenv.args.clone:
        check_if_clone_is_needed()
