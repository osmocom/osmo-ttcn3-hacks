...
class HttpClient {
    ...
    public:
	ResponseData postJsonWithCustomVerification(const std::string& url, unsigned int port, const std::string& jsonData, X509_STORE* store,
						    std::vector<X509*>& certPool) {
...
class RSPClient {
	...
	std::string sendHttpsPost(const std::string& endpoint, const std::string& body, int& httpStatusCode) {
		...
		if (m_useCustomTlsCert) {
			...
			response = m_httpClient->postJsonWithCustomVerification(url, m_serverPort, body, nullptr, rawCertPool);
		} else {
			std::vector<X509*> emptyCertPool;
			response = m_httpClient->postJsonWithCustomVerification(url, m_serverPort, body, nullptr, emptyCertPool);
