...
class CertificateUtil {
	...
	static std::vector<uint8_t> extractPublicKeyData(EVP_PKEY* key) {
		...
		if (EVP_PKEY_get_octet_string_param(key, OSSL_PKEY_PARAM_PUB_KEY, publicKeyData.data(), pub_len,
						    &pub_len) != 1) {
	...
	static bool verifyCertificateChainDynamic(X509* cert, const std::vector<X509*>& certPool,
		...
		std::unique_ptr<STACK_OF(X509), decltype(stack_only_deleter)> untrusted_chain(sk_X509_new_null(),
											      stack_only_deleter);
		if (!untrusted_chain) {
		...
		if (verbose) {
			LOG_INFO("Untrusted chain contains " + std::to_string(sk_X509_num(untrusted_chain.get())) +
				 " certificates");
		}
		...
		X509_STORE_CTX_set_verify_cb(ctx.get(), [](int ok, X509_STORE_CTX* ctx) -> int {
			if (!ok) {
				...
				if (error == X509_V_ERR_PERMITTED_VIOLATION || error == X509_V_ERR_EXCLUDED_VIOLATION ||
				    error == X509_V_ERR_SUBTREE_MINMAX) {
					// Get the certificate that triggered this
					X509* cert = X509_STORE_CTX_get_current_cert(ctx);
					if (cert) {
						char subject_buf[256];
						X509_NAME_oneline(X509_get_subject_name(cert), subject_buf,
								  sizeof(subject_buf));
						LOG_INFO("Processing certificate with name constraint abuse: " +
	...
	static bool xx_loadPublicKey(const std::string& pubKeyPath, const std::string& typeName,
		...
		try {
			...
			LOG_INFO("Loaded " + typeName + " public key from file (" +
				 std::to_string(publicKeyStorage.size()) + " bytes)");
			return true;

		} catch (const std::exception& e) {
			LOG_WARNING("Failed to load " + typeName + " public key from file: " + std::string(e.what()) +
				    " (will generate from private key)");
			return false;
		}
	}
	...
	static void xx_generatePublicKeyFromPrivate(const std::unique_ptr<EVP_PKEY, EVP_PKEY_Deleter>& privateKey,
		...
		LOG_DEBUG("Generated " + typeName + " public key from private key (" +
			  std::to_string(publicKeyStorage.size()) + " bytes)");
	}
	...
	static std::string getCurveOID(const std::vector<uint8_t>& certData) {
		try {
			...
			if (EVP_PKEY_get_utf8_string_param(pkey, "group", curve_name, sizeof(curve_name),
							   &curve_name_len) != 1) {
...
class RSPClient {
	...
	int getCertificateCurveNID(X509* cert) {
		...
		char curve_name[80];
		size_t curve_name_len = sizeof(curve_name);
		if (EVP_PKEY_get_utf8_string_param(pubkey.get(), OSSL_PKEY_PARAM_GROUP_NAME, curve_name,
	...
	void generateEUICCOtpk() {
		...
		if (EVP_PKEY_get_octet_string_param(m_euicc_ot_PrivateKey.get(), OSSL_PKEY_PARAM_PUB_KEY, nullptr, 0,
						    &pub_len) != 1) {
			throw OpenSSLError("Failed to get public key size");
		}
	...
	std::vector<uint8_t> computeECDHSharedSecret(const std::vector<uint8_t>& otherPublicKey) {
		...
		if (!OSSL_PARAM_BLD_push_octet_string(param_bld, OSSL_PKEY_PARAM_PUB_KEY, otherPublicKey.data(),
						      otherPublicKey.size())) {
			OSSL_PARAM_BLD_free(param_bld);
	...
	void setConfirmationCode(const std::string& confirmationCode) {
		...
		if (!m_transactionId.empty()) {
			...
			LOG_INFO("Set confirmation code, computed hash: " +
				 HexUtil::bytesToHex(m_confirmationCodeHash));
		} else {
		...
		for (const auto& cert : m_certPool) {
			...
			if (X509_check_issued(cert.get(), cert.get()) == X509_V_OK) {
				try {
					...
					LOG_WARNING("Failed to get curve type from certificate: " +
						    std::string(e.what()));
				}
			}
		}
	...
	void loadKeyPair(const std::string& privateKeyPath, const std::string& publicKeyPath,
			 const std::string& keyType, std::unique_ptr<EVP_PKEY, EVP_PKEY_Deleter>& privateKeyStorage,
			 std::vector<uint8_t>& publicKeyStorage) {
	...
	bool verifyServerSignature(const std::vector<uint8_t>& signedData, const std::vector<uint8_t>& signature,
				   X509* scertdata) {
		try {
	...
	std::string sendHttpsPostWithAuth(const std::string& endpoint, const std::string& body,
					  int& httpStatusCode, unsigned int portOverride) {
		if (!m_useMutualTLS) {
	...
	std::string sendHttpsPostUnified(const std::string& endpoint, const std::string& body,
		...
		LOG_DEBUG("Sending " + std::string(useMutualTLS ? "ES2+ request with mutual TLS" : "HTTPS request") +
			 " to: " + endpoint + " (port: " + std::to_string(portOverride) + ")");
		...
		LOG_DEBUG("HTTP " + std::to_string(httpStatusCode) +
