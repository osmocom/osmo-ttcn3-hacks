...
class RSPClient {
	...
	// Load SM-DP+ key pair and certificate (for simulation/testing purposes)
	void loadSMDPKeyAndCertificate(const std::string &smdpPrivateKeyPath, const std::string &smdpCertPath)
	{
		try {
			// Load SM-DP+ private key
			if (std::filesystem::exists(smdpPrivateKeyPath)) {
				std::ifstream keyFile(smdpPrivateKeyPath);
				if (!keyFile) {
					throw std::runtime_error("Failed to open SM-DP+ private key file: " +
								 smdpPrivateKeyPath);
				}

				std::string keyStr((std::istreambuf_iterator<char>(keyFile)),
						   std::istreambuf_iterator<char>());
				std::unique_ptr<BIO, BIODeleter> keyBio(BIO_new_mem_buf(keyStr.c_str(), -1));

				if (!keyBio) {
					throw OpenSSLError("Failed to create BIO for SM-DP+ private key");
				}

				m_smdpPrivateKey.reset(
					PEM_read_bio_PrivateKey(keyBio.get(), nullptr, nullptr, nullptr));
				if (!m_smdpPrivateKey) {
					throw OpenSSLError("Failed to read SM-DP+ private key");
				}

				Logger::info("Loaded SM-DP+ private key from: " + smdpPrivateKeyPath);
			} else {
				throw std::runtime_error("SM-DP+ private key file not found: " + smdpPrivateKeyPath);
			}

			// Load SM-DP+ certificate (CERT.DPpb.ECDSA)
			if (std::filesystem::exists(smdpCertPath)) {
				auto certChain = CertificateUtil::loadCertificateChain(smdpCertPath);
				if (!certChain.empty()) {
					m_smdpCertificate = std::move(certChain[0]);
					Logger::info("Loaded SM-DP+ certificate from: " + smdpCertPath);
					Logger::info("SM-DP+ certificate subject: " +
						     CertificateUtil::getSubjectName(m_smdpCertificate.get()));
				} else {
					throw std::runtime_error("No certificates found in SM-DP+ certificate file: " +
								 smdpCertPath);
				}
			} else {
				throw std::runtime_error("SM-DP+ certificate file not found: " + smdpCertPath);
			}

		} catch (const std::exception &e) {
			Logger::error("Failed to load SM-DP+ key and certificate: " + std::string(e.what()));
			throw;
		}
	}
  ...
  void debugASN1Structure(const std::vector<uint8_t>& derData) {
	...
	void loadEUICCCertificate(const std::string &euiccCertPath)
	{
		try {
			...
			auto euiccCerts = CertificateUtil::loadCertificateChain(euiccCertPath);
			if (euiccCerts.empty()) {
				throw std::runtime_error("No eUICC certificate found in file: " + euiccCertPath);
			}

			m_euiccCert = std::move(euiccCerts[0]);
			Logger::info("Loaded eUICC certificate: " + CertificateUtil::getSubjectName(m_euiccCert.get()));

			// Extract EID
			m_EID = CertificateUtil::getEID(m_euiccCert.get());

			if (m_EID.empty()) {
				Logger::warning("No EID found in eUICC certificate, using default");
				m_EID = "89049012345678901234567890123456"; // Default EID
			} else {
				Logger::info("Using EID from certificate: " + m_EID);
			}

			// Extract eUICC certificate SKI for use as PKIDs
			...
			if (!ski.empty()) {
				...
				Logger::warning("No SKI in eUICC certificate, using default PKIDs");
			}
		} catch (const std::exception &e) {
			Logger::error("Failed to load eUICC certificate: " + std::string(e.what()));
			throw;
		}
	}

	// Add to RSPClient class - generate or load eUICC key pair for signing
	void loadEUICCKeyPair(const std::string &euiccPrivateKeyPath)
	{
		try {
			if (std::filesystem::exists(euiccPrivateKeyPath)) {
				// Load existing private key
				std::ifstream keyFile(euiccPrivateKeyPath);
				if (!keyFile) {
					throw std::runtime_error("Failed to open eUICC private key file: " +
								 euiccPrivateKeyPath);
				}
				...
				if (!keyBio) {
					throw OpenSSLError("Failed to create BIO for eUICC private key");
				}

				m_euiccPrivateKey.reset(
					PEM_read_bio_PrivateKey(keyBio.get(), nullptr, nullptr, nullptr));
				if (!m_euiccPrivateKey) {
					throw OpenSSLError("Failed to read eUICC private key");
				}

				Logger::info("Loaded eUICC private key from: " + euiccPrivateKeyPath);
			} else {
				throw OpenSSLError("Failed to load EUICC priv key");
			}

			// Extract public key
			if (m_euiccPrivateKey) {
				extractEUICCPublicKey();
			}
		} catch (const std::exception &e) {
			Logger::error("Failed to load eUICC key pair: " + std::string(e.what()));
			throw;
		}
	}

	// Extract public key from private key
	void extractEUICCPublicKey()
	{
		if (!m_euiccPrivateKey) {
			throw std::runtime_error("No eUICC private key available");
		}

		// Get the EC key
		EC_KEY *ec_key = EVP_PKEY_get1_EC_KEY(m_euiccPrivateKey.get());
		if (!ec_key) {
			throw OpenSSLError("Failed to extract EC key");
		}
		...
		if (!pub_point || !group) {
			EC_KEY_free(ec_key);
			throw OpenSSLError("Failed to get public key point or group");
		}
		...
		m_euiccPublicKeyData.resize(pub_len);
		if (EC_POINT_point2oct(group, pub_point, POINT_CONVERSION_UNCOMPRESSED, m_euiccPublicKeyData.data(),
				       pub_len, nullptr) != pub_len) {
			EC_KEY_free(ec_key);
			throw OpenSSLError("Failed to extract public key data");
		}

		EC_KEY_free(ec_key);
		Logger::info("Extracted eUICC public key (" + std::to_string(pub_len) + " bytes)");
	}
...
// // openssl x509 -in cert_c/CERT_EUICC_VARC_SIG_NIST.der -inform DER -noout -ext subjectKeyIdentifier,authorityKeyIdentifier
// // openssl x509 -in cert_c/CERT_EUMSubCA_VARC_SIG_NIST.der -inform DER -noout -ext subjectKeyIdentifier,authorityKeyIdentifier
// // openssl x509 -in cert_c/CERT_EUM_VARC_SIG_NIST.der -inform DER -noout -ext subjectKeyIdentifier,authorityKeyIdentifier
// // openssl x509 -in cert_c/CERT_CISubCA_SIG_NIST.der -inform DER -noout -ext subjectKeyIdentifier,authorityKeyIdentifier
// // openssl x509 -in cert_c/CERT_CI_SIG_NIST.der -inform DER -noout -ext subjectKeyIdentifier,authorityKeyIdentifier
