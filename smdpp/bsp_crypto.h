...
class BspCrypto {
	...
	static std::vector<uint8_t> compute_cmac(const std::vector<uint8_t>& key, const std::vector<uint8_t>& data,
						 size_t output_size = AES_BLOCK_SIZE);

	std::vector<uint8_t> aes_cipher_operation(const std::vector<uint8_t>& input, const std::vector<uint8_t>& key,
    ...
    public:
	BspCrypto(const std::vector<uint8_t>& s_enc_key, const std::vector<uint8_t>& s_mac_key,
	...
	static BspCrypto from_kdf(const std::vector<uint8_t>& shared_secret, uint8_t key_type, uint8_t key_length,
	...
	std::vector<uint8_t> aes_decrypt_with_icv(const std::vector<uint8_t>& ciphertext,
	...
	BppProcessingResult process_bound_profile_package(const std::vector<uint8_t>& firstSequenceOf87,
							  const std::vector<uint8_t>& sequenceOf88,
							  const std::vector<uint8_t>& secondSequenceOf87,
							  const std::vector<uint8_t>& sequenceOf86);

	void reset(const std::vector<uint8_t>& initial_mcv);

	static std::vector<uint8_t> generate_replace_session_keys_data(const std::vector<uint8_t>& ppk_enc,
