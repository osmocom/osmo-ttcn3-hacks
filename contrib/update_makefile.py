...
def new_str_generate():
    ...
    for lib in libraries_list:
        name = f"lib_{lib['name']}"
        source = os.path.relpath(lib["sources"][0], ttcn3_hacks_dir)
        ...
        if name in targets_done:
            continue
        targets_done += [name, o_name]
        ...
        ret += f"\t\t{source} \\\n"
        for source_dep in lib["sources_deps"]:
            if source_dep.endswith(".ttcnpp"):
                source_dep_noext = os.path.splitext(source_dep)[0]
                source_dep = f"{source_dep_noext}.ttcn"
                ret += f"\t\t$(BUILDDIR)/{os.path.relpath(source_dep, ttcn3_hacks_dir)} \\\n"
            else:
                ret += f"\t\t{os.path.relpath(source_dep, ttcn3_hacks_dir)} \\\n"
        ret += "\t\t$(NULL)\n"
        ...
        ret += f"\t\t{source} \\\n"
        for source_dep in lib["sources_deps"]:
            # FIXME: deduplicate this with the sources list above
            if source_dep.endswith(".ttcnpp"):
                source_dep_noext = os.path.splitext(source_dep)[0]
                source_dep = f"{source_dep_noext}.ttcn"
                ret += f"\t\t$(BUILDDIR)/{os.path.relpath(source_dep, ttcn3_hacks_dir)} \\\n"
